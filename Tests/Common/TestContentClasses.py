...
class IntegrationConfiguration:
    def __init__(self, integration_configuration):
        ...
        self.is_byoi: bool = integration_configuration.get('byoi', False)
...
class TestConfiguration:
    def __init__(self, test_configuration: dict, default_test_timeout: int):
        ...
        self.test_integrations = self.parse_integrations_conf(test_configuration)
        self.test_instance_names = self.parse_instance_names_conf(test_configuration)

    @staticmethod
    def parse_integrations_conf(test_configuration):
    ...
    @staticmethod
    def parse_instance_names_conf(test_configuration):
...
class TestPlaybook:
    ...
    def __str__(self):
        return self.configuration.playbook_id
    ...
    def should_test_run(self):
        ...
        if self.build_context.filtered_tests and self.configuration.playbook_id not in self.build_context.filtered_tests:
            ...
            self.build_context.tests_data_keeper.skipped_tests.append(self.configuration.playbook_id)
            return False
        ...
        if self.configuration.playbook_id in self.build_context.conf.skipped_tests:
            ...
            if self.build_context.filtered_tests and self.configuration.playbook_id not in self.build_context.filtered_tests:
                self.build_context.logging_module.warning(f'Skipping test {self} because it\'s in skiiped test list')
            else:
                self.build_context.logging_module.debug(f'Skipping test {self} because it\'s in skiiped test list')
            self.build_context.tests_data_keeper.skipped_tests.append(self.configuration.playbook_id)
            return False
        ...
        for integration in self.configuration.test_integrations:
            ...
            if integration in self.build_context.conf.skipped_integrations.keys():
                self.build_context.logging_module.debug(
    ...
    def configure_integrations(self, client: DefaultApi) -> bool:
        ...
        for integration in self.integrations:
            instance_created = integration.create_integration_instance(client, self.configuration.playbook_id)
...
class BuildContext:
    def __init__(self, options, logging_module):
        ...
        self.conf, self.secret_conf = self.load_conf_files(self.conf_path, self.secret_conf_path)
        ...
        self.filtered_tests = self.extract_filtered_tests()
        self.tests_data_keeper = TestResults()
        self.unmockable_test_ids = self._get_unmockable_tests()
    ...
    def _get_tests_to_run(self) -> Tuple[Queue, Queue]:
        ...
        if self.server or not self.isAMI:
            unmockable_tests = self.specific_tests_to_run if self.specific_tests_to_run else all_tests
        elif self.isAMI:
            unmockable_tests = self._get_unmockable_tests()
            unmockable_test_ids = {test.playbook_id for test in unmockable_tests}
            self.logging_module.debug(f'Unmockable tests selected: {pformat(unmockable_test_ids)}')
            mockable_tests = [test for test in all_tests if test.playbook_id not in unmockable_test_ids]
    ...
    @staticmethod
    def extract_filtered_tests() -> list:
        """
    ...
    def _get_unmockable_tests(self) -> list:
        """
...
class Integration:
    def __init__(self, build_context: BuildContext, integration_name: str, potential_integration_instance_names: list):
        ...
        self.instance_name = ''
        self.configuration: Optional[IntegrationConfiguration] = None
    ...
    def _set_integration_params(self, server_url, playbook_id):
    ...
    def _get_integration_config(self, client: DefaultApi) -> Optional[dict]:
        ...
        """
        body = {
            'page': 0, 'size': 100, 'query': f'name:{self}'
        }
        try:
    ...
    def _delete_integration_instance_if_determined_by_name(self, client: DefaultApi, instance_name: str) -> None:
        ...
        if int(int_resp[1]) != 200:
            self.build_context.logging_module.error(f'Get integration instance failed with status code: {int_resp[1]}')
            return
        if 'instances' not in int_instances:
            self.build_context.logging_module.info('No integrations instances found to delete')
            return
    ...
    def create_integration_instance(self, client: DefaultApi, playbook_id: str) -> Optional[dict]:
        """
        ...
        """
        self._set_integration_params(client.api_client.configuration.host, playbook_id)
        ...
        self.build_context.logging_module.info(
            ...
            f'validate "Test": {self.configuration.should_validate_test_module})'
        )
    ...
    def disable_integrations_instances(self, client) -> None:
        ...
        self.build_context.logging_module.debug(f'Disabling integration {module_instance.get("name")}')
        try:
            res = demisto_client.generic_request_func(self=client, method='PUT',
                                                      path='/settings/integration',
                                                      body=module_instance)
    ...
    def get_docker_images(self) -> List[str]:
        ...
        """
        if self.integration_instance:
            return [image for image in Docker.get_integration_image(self.integration_instance) if image]
        else:
...
class TestContext:
    ...
    def _collect_docker_images(self) -> None:
        ...
        for integration in self.playbook.integrations:
            self.test_docker_images.update(integration.get_docker_images())
    ...
    def poll_for_playbook_state(self) -> str:
        """
    ...
    def _run_incident_test(self) -> str:
        ...
        playbook_state = self.poll_for_playbook_state()
    ...
    def incident_and_docker_test(self):
    ...
    def _execute_unmockable_test(self) -> bool:
        ...
        with acquire_test_lock(self.playbook) as lock:
            if lock:
                status = self.incident_and_docker_test()
    ...
    def __str__(self):
        ...
        integration_names = ', '.join([integration.name for integration in self.playbook.integrations])
        if self.playbook.integrations:
            test_message += f' with integration(s): {integration_names}'
        else:
...
class TestsExecution:
    ...
    def _execute_tests(self, queue: Queue) -> None:
        ...
        while queue.unfinished_tasks:
            try:
                ...
                self.check_if_test_was_executed_in_this_round(test_playbook, queue)
            except Empty:
                continue
            self._configure_new_client()
    ...
    def check_if_test_was_executed_in_this_round(self, test_playbook: TestPlaybook, queue: Queue) -> None:
        """
        ...
        Args:
            ...
            queue: The queue from which the current tests are executed
        """
        queue.mutex.acquire()
        if str(test_playbook.configuration) in self.build_context.executed_in_current_round:
            self.build_context.logging_module.info(
                'all tests in the queue were executed, sleeping for 30 seconds to let locked tests get unlocked.')
            self.build_context.executed_in_current_round = {str(test_playbook.configuration)}
            queue.mutex.release()
            time.sleep(30)
            return
        queue.mutex.release()
